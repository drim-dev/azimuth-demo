...
public static class TripEventReceiver
{
    public static async Task RunAsync(
        string uri,
        string queue,
        Func<TripStateChanged, CancellationToken, Task> apply,
        ILogger logger,
        CancellationToken cancellation)
    {
        while (!cancellation.IsCancellationRequested)
        {
            ...
            {
                logger.LogWarning(exception, "Trip lifecycle queue {Queue} is unavailable", queue);
            }
        ...
        CancellationToken cancellation)
    {
        ...
        {
            if (!TripStateChangedCodec.TryDeserialize(delivery.Body.Span, out var message))
            {
                logger.LogError("Rejected malformed trip lifecycle event {MessageId}",
            ...
            {
                logger.LogError(exception, "Deferred trip lifecycle event {EventId}", message!.EventId);
                await session.Channel.BasicNackAsync(
                    delivery.DeliveryTag,
                    multiple: false,
                    requeue: true,
