...
**A requirement is incompletely described until three things are recorded about it: what must be
true, what makes it true, and how we know (D3).** Recording only the first is what specification
practice does today; recording the first and third is what a traceability matrix does. The
framework's bet is that the second is load-bearing and its absence is why the other two rot.

The consequence that makes this checkable rather than editorial: **holes are generated by which
facets are missing**, so a finding is a structural fact about the model rather than a matter of
taste (D3).
...
**Exemption** generalizes the alpha's `Untraced`: a deliberate, attributable, reviewable opt-out is
fine anywhere, and a silent absence is not (D6.3).
